...
# 2. READ DATASETS #
####################
setwd("C:/Users/Usuario/Desktop/ICM/FEINA/Fran Ramírez/SteLar - Delta (Cory's)/VMS")
load("dades/CORYs_environment_MEDSEA.RData")
rm(list=setdiff(ls(), c("df_env_", "depth","npp_year", "sal_year", "tem_year")))


# 2.1. Eliminem mostres que estan fora de la GSA06.
gsa <- readShapePoly("C:/Users/Usuario/Desktop/ICM/FEINA/Fran Ramírez/SteLar - Delta (Cory's)/VMS/gsa/GSAs_simplified.shp",
                     proj4string=CRS("+init=epsg:4326"))
gsa06 <- subset(gsa, gsa@data$SECT_COD %in% c("GSA06"))


gsa_sf <- as(st_as_sf(x = df_env_, coords = c("lon", "lat"),
                      crs="+proj=longlat +datum=WGS84"),"Spatial")
gsa_sf <- gsa_sf[gsa06,]
df_env_ <- as.data.frame(gsa_sf)
remove(list=c("gsa", "gsa_sf"))


# 2.2. Eliminem observacions amb NAs a les variables ambientals. 
## Passem de 144022 files a 141178.
df_env_ <- df_env_[complete.cases(df_env_[6:9]),] 


# 2.3. Separem data frame per espècies i deixem únicament variables necessàries.
...
# 2.4. Creem columna de presencia/absència (1/0).
...
# 2.5. Creem dataframes amb només valors positius.
...
# 2.6. Guardem les dades en un nou workspace.
setwd("C:/Users/Usuario/Desktop/ICM/FEINA/Fran Ramírez/SteLar - Delta (Cory's)/VMS")
save.image(file = "dades/data.RData")
load("C:/Users/Usuario/Desktop/ICM/FEINA/Fran Ramírez/SteLar - Delta (Cory's)/VMS/dades/data.RData")
...
plot(spain, xlim=c(-1,4), ylim=c(37,43), axes=TRUE,col='lightgray')
...
p1 <- ggplot(df1, aes(Year, Biomass)) + 
  ...
  scale_x_continuous(breaks=c(2008,2013,2019))
...
brt.ber.anx <- gbm.step(data= sample_anchovy, gbm.x = c(1,5:8), gbm.y = 9, 
                        tree.complexity=1,tolerance.method='fixed',
...
setwd("C:/Users/Usuario/Desktop/ICM/FEINA/Fran Ramírez/SteLar - Delta (Cory's)/VMS")
save.image(file = "models_VMS.RData")
load("C:/Users/Usuario/Desktop/ICM/FEINA/Fran Ramírez/SteLar - Delta (Cory's)/VMS/models_VMS.RData")
...
rm(list=c("devexpl.ber.anx", "devexpl.ber.sar", "devexpl.ber.sardinella"))
...
brt.gau.sardinella <- gbm.step(data= sample_sardinella_bio, gbm.x = c(1,5:8), 
                               gbm.y = 9, tree.complexity=1, silent=T,
...
rm(list=c("devexpl.gau.anx", "devexpl.gau.sar", "devexpl.gau.sardinella"))
...
nb <- dnearneigh(as.matrix(coords), 0, 10)
listw <- nb2listw(nb)
...
zepa <-  readShapePoly("C:/Users/Usuario/Desktop/ICM/FEINA/Fran Ramírez/SteLar - Delta (Cory's)/VMS/gsa/ZEPA_Ebre_WGS84.shp",
                       proj4string=CRS("+init=epsg:4326"))
...
setwd("C:/Users/Usuario/Desktop/ICM/FEINA/Fran Ramírez/SteLar - Delta (Cory's)/VMS")
load("C:/Users/Usuario/Desktop/ICM/FEINA/Fran Ramírez/SteLar - Delta (Cory's)/SVM/models_VMS.RData")
